...
A complete modern rebuild of [swim-montreal.com](https://swim-montreal.com) — 10 fully self-contained HTML pages, consistent design system, animated water backgrounds, live search, interactive features, and a chatbot UI throughout.
...
| `jobs.html` | Jobs & Careers | Full instructor recruitment page — perks, MISway method, testimonials, apply steps |
...
| `contact.html` | Contact & Locations | Contact form, social links, 18 clickable Google Maps location cards with cursor glow effect |
| `faq.html` | FAQ | 12 real FAQs with live search, category tabs, accordion |
| `pricay-policy.html` | Policies | House rules, cancellation, payment, pool-specific rules, at-home policies — sticky sidebar TOC |
| `news.html` | Latest News | 9 real blog posts with category filter chips and newsletter signup |
| `lifeguard.html` | Private Lifeguard | Full-height hero, what we do, what to expect, MIS advantage, booking steps |
...
- **Animated water canvas** — bubbles, light rays, layered waves on every hero section
- **Live search** — meet-the-team and FAQ filter in real time as you type
- **Category filters** — filter instructors by specialty, news by topic, FAQs by category
...
- **Consistent footer** — all 9 other pages linked from every footer
...
### GitHub Pages
1. Upload all 10 `.html` files to the root of a new GitHub repo
...
3. Drag and drop the folder containing all 10 files
...
The chatbot UI is built into every page and routes API calls to `/api/chat`. To enable it you need a simple serverless proxy that holds your Anthropic API key securely.

**Netlify Function example** (`netlify/functions/chat.js`):
```js
...
Set `ANTHROPIC_API_KEY` as an environment variable in your Netlify dashboard. Add a `netlify.toml` redirect:
